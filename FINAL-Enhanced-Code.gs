// ============================================
// NoLossForm.com - FINAL ENHANCED Google Apps Script
// VERSION: 3.4 - ENHANCED WITH AGENCY INFO & TIME
...
// ============================================
// ENHANCED PDF WITH AGENCY INFO & TIME
// ============================================
...
function generateCompletePDF(data, confirmationNumber) {
  try {
    ...
    // Row 3: Dates WITH TIME for reinstatement
    const row3 = infoTable.appendTableRow();
    row3.appendTableCell('LAPSE DATE:').getChild(0).asParagraph().setBold(true).setFontSize(8);
    ...
    // Add time to reinstatement date - default to 12:01 AM if no time specified
    const reinstatementWithTime = formatDateWithTime(data.reinstatementDate || data.lapseEndDate);
    row3.appendTableCell(reinstatementWithTime).getChild(0).asParagraph().setFontSize(8);
    ...
    // Row 5: Amount Paid (if provided)
    if (data.amountPaid) {
      const row5 = infoTable.appendTableRow();
      row5.appendTableCell('AMOUNT:').getChild(0).asParagraph().setBold(true).setFontSize(8);
      row5.appendTableCell(data.amountPaid).getChild(0).asParagraph().setFontSize(8);
      row5.appendTableCell('').getChild(0).asParagraph().setFontSize(8);
      row5.appendTableCell('').getChild(0).asParagraph().setFontSize(8);
    }
    ...
    // Legal text with reinstatement time included
      ...
      <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px; background: #f9f9f9;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr><td style="padding: 8px;"><strong>Reinstatement Date/Time:</strong></td><td>${formatDateWithTime(data.reinstatementDate || data.lapseEndDate)}</td></tr>
          </table>
    ...
    const htmlBody = `
      ...
      <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px; background: #f9f9f9;">
          ...
          <p>This statement confirms that no loss, damage, or claim occurred during your coverage lapse period from <strong>${formatDate(data.cancellationDate || data.lapseStartDate)}</strong> to <strong>${formatDateWithTime(data.reinstatementDate || data.lapseEndDate)}</strong>.</p>
